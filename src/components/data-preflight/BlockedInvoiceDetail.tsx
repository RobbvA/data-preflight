...
export function BlockedInvoiceDetail({
  ...
  return (
    <section className="rounded-2xl border border-white/[0.08] bg-slate-950/55 p-4 shadow-xl shadow-slate-950/25 backdrop-blur">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-white/[0.06] pb-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-red-200/70">
            Inspection mode
          </p>
        ...
        <button
          type="button"
          onClick={onClose}
          className="rounded-full border border-white/[0.08] bg-white/[0.03] px-3 py-1 text-xs font-medium text-slate-300 transition hover:border-white/15 hover:bg-white/[0.06] hover:text-white"
        >
          Close detail
        </button>
      </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[0.9fr_1.3fr]">
        <div className="rounded-xl border border-white/[0.07] bg-slate-950/45 p-3">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-sm font-semibold text-slate-200">
              Normalized invoice data
            </h3>

            <span className="rounded-full border border-white/[0.06] bg-white/[0.02] px-2 py-0.5 text-[10px] text-slate-500">
              row {selectedBlockedInvoice.rowIndex}
            </span>
          </div>

          <pre className="mt-3 max-h-[360px] overflow-auto rounded-lg bg-slate-950/70 p-3 text-[11px] leading-5 text-slate-300">
        ...
        <div className="rounded-xl border border-white/[0.07] bg-slate-950/35 p-3">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <span className="rounded-full border border-red-400/20 bg-red-500/[0.07] px-2 py-0.5 text-[10px] font-medium text-red-100/90">
              {selectedBlockedInvoice.issues.length} issue
              {selectedBlockedInvoice.issues.length === 1 ? "" : "s"}
            </span>
          </div>

          <div className="divide-y divide-white/[0.06] rounded-lg border border-white/[0.06] bg-slate-950/35">
            {selectedBlockedInvoice.issues.map((issue) => (
              ...
              >
                ...
                <span className="h-fit rounded-full border border-white/[0.06] bg-slate-900/40 px-1.5 py-0.5 text-[9px] text-slate-600">
