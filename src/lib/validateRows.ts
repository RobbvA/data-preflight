...
type InvoiceValidationRule = {
  id: string;
  description: string;
  validate: (context: InvoiceValidationContext) => ValidationIssue[];
};
...
const invoiceValidationRules: InvoiceValidationRule[] = [
  {
    id: "empty-row",
    description: "Detect rows without usable values.",
    validate: validateEmptyRow,
  },
  {
    id: "required-fields",
    description: "Detect missing required invoice fields.",
    validate: validateRequiredFields,
  },
  {
    id: "email",
    description: "Validate invoice email addresses.",
    validate: validateEmail,
  },
  {
    id: "amount",
    description: "Validate invoice amount values.",
    validate: validateAmount,
  },
  {
    id: "vat",
    description: "Validate VAT values.",
    validate: validateVat,
  },
  {
    id: "vat-consistency",
    description: "Validate VAT against invoice amount.",
    validate: validateVatConsistency,
  },
  {
    id: "duplicate-invoice-number",
    description: "Detect duplicate invoice references.",
    validate: validateDuplicateInvoiceNumber,
  },
  {
    id: "status",
    description: "Validate invoice workflow status.",
    validate: validateStatus,
  },
  {
    id: "country",
    description: "Validate country values.",
    validate: validateCountry,
  },
  {
    id: "currency",
    description: "Validate currency values.",
    validate: validateCurrency,
  },
  {
    id: "invoice-date",
    description: "Validate invoice date.",
    validate: validateInvoiceDate,
  },
  {
    id: "due-date",
    description: "Validate due date.",
    validate: validateDueDate,
  },
  {
    id: "payment-terms",
    description: "Validate payment term date relationship.",
...
export function validateRows(rows: ParsedRow[]): ValidationResult {
  ...
  const issues = rows.flatMap((row, index) => {
    ...
    return runInvoiceValidationRules(context);
  });
...
function runInvoiceValidationRules(
  context: InvoiceValidationContext,
): ValidationIssue[] {
  return invoiceValidationRules.flatMap((rule) => rule.validate(context));
}
  ...
  if (normalizedRow.amount && amount === null) {
    issues.push(
      createIssue({
        ...
        why: "Accounting and reporting systems require numeric invoice amounts for totals, balances, revenue calculations, and reconciliation.",
  ...
  if (amount !== null && amount <= 0) {
    issues.push(
      createIssue({
        ...
        fix: "Verify whether this row should be treated as a normal invoice or as a separate financial flow.",
        severity: "warning",
        risk: "financial_reporting",
      }),
    );
  ...
  if (vat > amount) {
    issues.push(
      createIssue({
        ...
        why: "VAT higher than the invoice amount usually indicates incorrect exports, swapped fields, or broken field mapping.",
  ...
  if (vatRate > 0.3) {
    issues.push(
      createIssue({
        ...
        why: "A VAT rate above 30% is uncommon for normal invoice flows and may indicate incorrect mapping or calculation.",
        fix: "Check whether the amount includes VAT, whether columns were swapped, or whether this row uses a special tax rule.",
        severity: "warning",
        risk: "tax_risk",
      }),
    );
...
function validateDuplicateInvoiceNumber({
  ...
  return [
    createIssue({
      ...
      why: "Duplicate invoice numbers can create duplicate records, double processing, overwrites, duplicate payments, or reconciliation conflicts.",
...
function validateCountry({
  ...
  if (!country) {
    return [
      createIssue({
        ...
        why: "Country affects tax handling, regional compliance, routing logic, and reporting segmentation.",
  ...
  return [
    createIssue({
      ...
      why: "Invalid country codes can create incorrect regional classification, tax assumptions, or failed imports.",
...
function validateCurrency({
  ...
  if (!currency) {
    return [
      createIssue({
        ...
        why: "Missing currency makes invoice totals ambiguous in reporting and reconciliation workflows.",
  ...
  return [
    createIssue({
      ...
      why: "Invalid currencies can create failed imports, incorrect reporting, or incorrect financial interpretation.",
...
function validateInvoiceDate({
  ...
  return [
    createIssue({
      ...
      why: "Invalid invoice dates can break accounting periods, reporting, VAT periods, and audit trails.",
...
function validateDueDate({
  ...
  return [
    createIssue({
      ...
      why: "Invalid due dates break payment tracking, reminders, aging reports, and cash-flow planning.",
      fix: "Use a valid YYYY-MM-DD date.",
      severity: "warning",
      risk: "payment_terms",
    }),
...
function validatePaymentTerms({
  ...
  return [
    createIssue({
      ...
      why: "This usually indicates swapped date fields, invalid payment terms, or source export errors.",
